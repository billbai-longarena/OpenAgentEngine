...
11. Run the phase quality gate (single command for workspace + S-005 + S-006 + S-007 + S-010 + S-011 + S-013 + S-014 + S-015 + S-018 checks):
```bash
...
13. Start local infra baseline (PostgreSQL + Redis + NATS):
```bash
docker compose -f infra/docker/docker-compose.yml up -d
```

14. Optional workspace commands (after installing deps):
```bash
pnpm install
pnpm run lint
pnpm run build
